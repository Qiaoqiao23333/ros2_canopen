...
bool Motor402::switchState(const State402::InternalState & target)
{
  ...
  while (state != target_state_)
  {
    ...
    {
      // Provide more context for field debugging.
      uint16_t sw = 0;
      try
      {
        sw = driver->universal_get_value<uint16_t>(status_word_entry_index, 0x0);
      }
      catch (...)
      {
      }
      const auto target_state = target_state_.load();
      RCLCPP_INFO(
        rclcpp::get_logger("canopen_402_driver"),
        "Transition timed out. target=%d next=%d current=%d statusword=0x%04x",
        static_cast<int>(target_state), static_cast<int>(next), static_cast<int>(state), sw);
      if (enable_diagnostics_.load())
      {
        this->diag_collector_->addf(
          "cia402_state", "State transition timed out: %d -> %d (sw=0x%04x)", state, next, sw);
      }
      return false;
    }
  }
...
bool Motor402::handleInit()
{
  ...
  // NOTE: In many real setups homing requires external conditions (limit switches,
  // operator confirmation, etc.). Running it automatically during bringup can cause
  // ros2_control activation to fail.
  // If homing is required, run it explicitly after bringup.
  return true;
}
