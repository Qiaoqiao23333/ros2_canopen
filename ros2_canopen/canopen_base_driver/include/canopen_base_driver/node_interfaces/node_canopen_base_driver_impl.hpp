        ...
        catch (const std::exception & e)
        {
          ...
          if (boot_attempts < max_boot_attempts)
          {
            ...
            catch (const std::exception & nmt_e)
            {
              RCLCPP_WARN(
                this->node_->get_logger(),
                "Failed to send NMT reset command: %s. Continuing with boot retry anyway.",
                nmt_e.what());
            }
            try
            {
            ...
            catch (const std::exception & boot_e)
            {
              RCLCPP_WARN(
                this->node_->get_logger(),
                "Failed to trigger boot retry: %s. Will retry in next iteration.",
                boot_e.what());
