...
public:
  ...
  {
    // Check if the object is mapped to a TPDO
    bool is_tpdo = false;
    if (this->pdo_map_->find(index) != this->pdo_map_->end())
    {
      const auto & object = this->pdo_map_->at(index);
      if (object.find(subindex) != object.end())
      {
        is_tpdo = object.at(subindex).is_tpdo;
      }
    }

    // If mapped to TPDO, prefer the cached value from the local dictionary
    // EXCEPTION: Always use SDO for StatusWord (0x6041) to ensure robust state machine transitions
    // during initialization, as TPDOs might not be active yet or might be delayed.
    if (is_tpdo && index != 0x6041)
    {
      std::scoped_lock<std::mutex> lck(this->dictionary_mutex_);
      ...
      // If type doesn't match known types, fall through or return 0 (should not happen for standard types)
    }

    do_sdo:
    // Robust behaviour: always prefer a fresh SDO read when polling.
    // PDO-mapped reads depend on TPDO traffic (SYNC / event-driven). If TPDO updates
    // are missing, the cached dictionary value becomes stale and CiA402 state
    // transitions can time out.
    T value = 0;
    ...
    // Fallback: return the last cached value from the local dictionary (updated via TPDO/RPDO).
    std::scoped_lock<std::mutex> lck(this->dictionary_mutex_);
    if (typeid(T) == typeid(uint8_t))
    {
  ...
  void universal_set_value(uint16_t index, uint8_t subindex, T value)
  {
    ...
    if (is_rpdo)
    {
      // Remote object is mapped into an RPDO (master -> slave). In Lely, this means:
      // write the value into the *local TPDO-mapped* mirror and then trigger a TPDO event.
      // If mapping is not ready yet, fall back to SDO instead of crashing.
      {
        std::scoped_lock<std::mutex> lck(this->dictionary_mutex_);
      ...
      {
        // Happens if the object is not (yet) mapped in the local PDO mapping.
        // SDO is slower but prevents hard failure during startup.
